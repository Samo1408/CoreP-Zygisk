...
struct Api {
    ...
    bool pltHookCommit();
private:
    struct internal_api_table;
    internal_api_table *impl;
    template<class T> friend void internal_entry(internal_api_table*, JNIEnv*);
};
namespace internal {
struct module_abi {
...
template<class T> void entry_impl(api_table*table,JNIEnv*env){
    static Api api; api.impl=reinterpret_cast<Api::internal_api_table*>(table);
