...
import org.lsposed.corepatch.ZygiskHelper.log
...
object PackageManagerServiceUtilsHook : BaseHook() {
    ...
    override fun hook() {
        ...
        if (!ZygiskHelper.deoptimize(verifySignaturesMethod)) log("failed to deoptimize verifySignatures")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            if (!ZygiskHelper.deoptimize(canJoinSharedUserIdMethod)) log("failed to deoptimize canJoinSharedUserId")
        }
    }
}
